...
describe("computeWaveformSegments", () => {
  ...
  test("1:1 mapping — one frame per pixel, single segment", () => {
    ...
    expect(segments[0].rpStart).toBe(0);
    expect(segments[0].rpLength).toBe(120);
  ...
  test("partitioning — splits into max 512px segments", () => {
    ...
    expect(segments[0].rpStart).toBe(0);
    expect(segments[0].rpLength).toBe(512);
    expect(segments[1].rpStart).toBe(512);
    expect(segments[1].rpLength).toBe(512);
    expect(segments[2].rpStart).toBe(1024);
    expect(segments[2].rpLength).toBe(476);
