...
function collectAllEvents(playback: ReturnType<typeof createPlayback>): PlaybackEvent[] {
  ...
  while (newEvents.length > 0) {
    ...
    newEvents = playback.getEvents(events[events.length - 1]!.triggerChartTime + 999);
  }
  return events;
}
...
describe("createPlayback", () => {
  test("single sound event at cursor produces one event at triggerChartTime 0", () => {
  ...
  test("sound event after cursor has correct triggerChartTime offset", () => {
    const playback = createPlayback({
    ...
    const events = collectAllEvents(playback);
    expect(events[0].triggerChartTime).toBeCloseTo(0.5, 2);
  });
  ...
  test("multiple events on same channel produce correct times", () => {
    ...
    expect(events[0].triggerChartTime).toBe(0);
    expect(events[1].triggerChartTime).toBeCloseTo(1, 2);
  });
  ...
  test("play after continue resets the chain", () => {
    ...
    const events = collectAllEvents(playback);
    expect(events).toHaveLength(2);
    expect(events[0]).toMatchObject({ triggerChartTime: 0, audioStartTime: 0 });
    expect(events[1]).toMatchObject({ triggerChartTime: 2, audioStartTime: 0, audioEndTime: 5 });
  });
  ...
  test("different sound lanes are independent", () => {
    ...
    expect(events).toHaveLength(2);
    const sorted = [...events].sort((a, b) => a.triggerChartTime - b.triggerChartTime);
    expect(sorted[0].triggerChartTime).toBe(0);
    expect(sorted[1].triggerChartTime).toBe(0);
  });
  ...
  test("join mid-stream: cursor starts between play and continue", () => {
    ...
    expect(events[0]).toMatchObject({
      triggerChartTime: 0,
      audioStartTime: 0.5, // halfway through the file
      audioEndTime: 5,
    });
  });
  ...
  test("join mid-stream for long audio that extends past last event", () => {
    ...
    expect(events[0]).toMatchObject({
      triggerChartTime: 0,
      audioStartTime: 1, // 1s of the 10s audio has already "played"
      audioEndTime: 10,
    });
  });
  ...
  test("incremental getEvents dequeues correctly", () => {
    ...
    // First chain (play at 0 + continue at 240) coalesced at triggerChartTime 0
    const batch1 = playback.getEvents(0.6);
    expect(batch1).toHaveLength(1);

    // Second chain (play at 480) at triggerChartTime 1.0
